...
_tmuxcfg () {
  if ! command -v tmux &>/dev/null ; then
    echo "tmux not found, skipping tmux config"
    return
  fi
  if [[ -e "$HOME/.tmux.conf" ]] ; then
    echo "tmux config already present, skipping tmux config"
    return
  fi
...
_chromaconfig () {
  ...
  if [[ ! -e "$HOME/.bin/chroma" ]]; then
    chroma_release="https://github.com/alecthomas/chroma/releases/download/v2.14.0/chroma-2.14.0-linux-amd64.tar.gz"
    # use curl or wget to download chroma_release and extract only the chroma binary to $HOME/.bin
    if command -v curl &>/dev/null; then
      curl -sSL "$chroma_release" | tar -xz -C "$HOME/.bin" chroma
    elif command -v wget &>/dev/null; then
      wget -qO- "$chroma_release" | tar -xz -C "$HOME/.bin" chroma
    else
      echo "curl or wget not found, skipping chroma install"
      return
    fi
  fi
